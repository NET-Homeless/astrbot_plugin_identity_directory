...
- `/whoami`（`/我是谁`）— 显示当前账号在通讯录里的身份与绑定账号数。
- `/directory`（`/通讯录` / `/通讯录统计`）— 查看通讯录联系人、账号与名片统计数据。
- `/lookup <名字>`（`/查人 <名字>`）— 按显示名或群名片搜索联系人及关联账号。
- `/link`（`/bind` / `/绑定`）— 跨平台自助绑定：发送 `/link` 获取 10 分钟一次性绑定码；在另一个平台发送 `/link <绑定码>` 即可自助合并绑定两个账号。
## 其他插件调用
